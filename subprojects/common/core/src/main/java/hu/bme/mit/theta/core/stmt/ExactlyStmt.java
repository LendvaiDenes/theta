...
public final class ExactlyStmt implements Stmt {

	private static final int HASH_SEED = 929;
	private static final String STMT_LABEL = "exactly";
	...
	private ExactlyStmt(final List<VarDecl<BoolType>> varDecls,final Expr<IntType> sum) {
		this.varDecls = checkNotNull(varDecls);
		this.sum = checkNotNull(sum);
	}

	public static ExactlyStmt of(final List<VarDecl<BoolType>> varDecls, final Expr<IntType> sum) {
		return new ExactlyStmt(varDecls, sum);
	}
	...
	public int hashCode() {
		...
		if (result == 0) {
			result = HASH_SEED;
			result = 35 * result + varDecls.hashCode() + sum.hashCode();
			hashCode = result;
		}
		return result;
	}
	...
	public boolean equals(final Object obj) {
		if (this == obj) {
			return true;
		} else if (obj instanceof ExactlyStmt) {
			final ExactlyStmt that = (ExactlyStmt) obj;
